...
namespace LoginRegister.Controllers
{
    ...
    public class WallController : Controller
    {
        ...
        public IActionResult Comment(string id)
        {
            ...
            string query = $@"SELECT messages.id AS message_id, messages.message, messages.created_at, users.first_name, users.last_name 
                             FROM messages JOIN users ON messages.user_id = users.id WHERE messages.id = {id}";

            string comment_query = $@"SELECT comments.id AS comment_id, comments.message_id, comments.comment, comments.created_at, users.first_name, users.last_name 
        ...
        public IActionResult Create(IndexView wallModel)
        {
            ...
            {
                string insert_query = $@"INSERT INTO messages(message, user_id, created_at, updated_at) 
        ...
        public IActionResult CreateComment(IndexView commentModel)
        {
            ...
            if(ModelState.IsValid)
            {
                string insert_query = $@"INSERT INTO comments(comment, message_id, user_id, created_at, updated_at) 
                                VALUES('{commentModel.NewComment.comment}',{commentModel.NewComment.message_id},
